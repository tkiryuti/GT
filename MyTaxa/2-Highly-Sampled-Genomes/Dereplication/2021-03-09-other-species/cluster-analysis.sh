...
## What is the dereplication directory?
derep_dir="$1"
if [[ ! -d "$derep_dir" ]]; then
	echo "Error: dereplication result directory does not exist. Attempted the following path:"
	echo "$derep_dir"
	exit 1
fi
cd "$derep_dir"

## Make analysis dir if it doens't exist
